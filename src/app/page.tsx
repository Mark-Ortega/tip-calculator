...
export default function Home() {
  ...
  useEffect(() => {
    if (selectedValue != '' && dollarInput > 0 && peopleInput > 0) {
      const percentage = parseFloat(selectedValue.replace('%', '')) / 100;
      console.log(percentage)

      setTip((dollarInput * percentage) / peopleInput);
      setTotal((dollarInput + (dollarInput * percentage)) / peopleInput);
    }
  }, [selectedValue, dollarInput, peopleInput]);
      ...
      <div className="mt-10 md:mt-16 rounded-xl bg-white p-6 text-grayCyan flex justify-center flex-wrap gap-10 text-lg sm:text-sm">
        <div className="ps-0 md:ps-4 pt-3 w-80 md:max-w-fit">
          ...
          <div className="pt-10 pb-1 flex justify-between">
            ...
            <p className={`text-orange-600 ${selectedValue != '' && dollarInput > 0 && peopleInput == 0 ? 'block' : 'hidden'}`}>Cant be zero</p>
          </div>
        ...
        <div className="bg-darkCyan pt-10 pb-6 px-7 sm:px-10 text-white rounded-xl flex flex-col justify-between">
          ...
          <button onClick={handleReset} className={`mt-10 sm:pt-0 ${selectedValue == '' && dollarInput == 0 && peopleInput == 0 ? 'bg-[#006b70]': 'bg-strongCyan'} hover:bg-lightGrayCyan hover:text-darkCyan flex justify-center items-center h-12 sm:h-8 rounded`}>
